...
def decode_bits(bits):
    ...
    #clear 0s in front and back
    bits = bits.lstrip("0").rstrip("0")
    
    #separate 0s and 1s into different lists within a list
    previous = "1"
    listify = [[]]
    count = 0
    for i in bits:
    ...
    #find minum list length and use it as the timeunit
    timeunit = len(min(listify, key=len))
    
    #transform into dots and dashes
    dad= ""
    for i in listify:
        if sum(i) == 0 and len(i) == timeunit:
            dad += ""
        elif sum(i) == 0  and len(i) == timeunit*3:
            dad += " "
        elif sum(i) == 0  and len(i) == timeunit*7:
            dad += "   "
        elif sum(i) != 0  and len(i) == timeunit*3:
            dad += "-"
        else:
...
def decode_morse(morse_code):
    ...
    #remove spaces before the first code
